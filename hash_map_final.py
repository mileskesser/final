...
class HashMap:
    ...
    def resize_table(self, capacity):
        ...
        # Iterate over the buckets in the existing hash table
        for index in range(self._buckets):
            # Set cur to index
            cur = index
            # While cur is a node in the linked list
            while cur is not None:
                # Get key
                key = cur.key
                # Get value of key
                value = self._buckets.get(key)
                # Calculate index
                hash = temp._hash_function(key) % temp.capacity
                # Add key,value to front of linked list
                temp[hash].add_front(key, value)
                # Update cur to next node in linked list
                cur = cur.next
        # Point hashMap to temp hashMap
